        ...
        controller: ['$scope', '$element', '$attrs', 'crudGridDataFactory', 'notificationFactory',
            function ($scope, $element, $attrs, crudGridDataFactory, notificationFactory) {
                ...
                $scope.addAlert = function () {
                    $scope.alerts.push({ type: 'error', msg: 'Please enter a grade between 1 and 100' });
                };
                ...
                $scope.validateGradeAdd = function () {
                    $scope.alerts = [];
                    if (student.grade.value < 1 || student.grade.value > 100) {
                        $scope.addAlert();
                    }
                    else { $scope.addObject(); }
                };

                $scope.validateGradeEdit = function (object) {
                    $scope.alerts = [];
                    if (object.Grade < 1 || object.Grade > 100) {
                        $scope.addAlert();
                    }
                    else { $scope.updateObject(object); }
                };
