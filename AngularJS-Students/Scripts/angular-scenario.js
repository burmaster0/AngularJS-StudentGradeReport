 ...
 * @example
   <doc:example>
     <doc:source>
    <div ng-init="greeting='Hello'; Student='World'">
      {{greeting}} {{Student}}!
    </div>
     </doc:source>
     <doc:scenario>
       it('should check greeting', function() {
         ...
         expect(binding('Student')).toBe('World');
       });
     </doc:scenario>
   </doc:example>
 */
 ...
 *
 * <pre>
 * <ng-pluralize count="StudentCount"
                 when="{'0': 'Nobody is viewing.',
 *                      'one': '1 Student is viewing.',
 *                      'other': '{} Students are viewing.'}">
 * </ng-pluralize>
 *</pre>
 *
 * In the example, `"0: Nobody is viewing."` is an explicit number rule. If you did not
 * specify this rule, 0 would be matched to the "other" category and "0 Students are viewing"
 * would be shown instead of "Nobody is viewing". You can specify an explicit number rule for
 * other numbers, for example 12, so that instead of showing "12 Students are viewing", you can
 * show "a dozen Students are viewing".
 *
 * You can use a set of closed braces(`{}`) as a placeholder for the number that you want substituted
 * into pluralized strings. In the previous example, Angular will replace `{}` with
 * <span ng-non-bindable>`{{StudentCount}}`</span>. The closed braces `{}` is a placeholder
 * for <span ng-non-bindable>{{numberExpression}}</span>.
 *
 * # Configuring ngPluralize with offset
 * The `offset` attribute allows further customization of pluralized text, which can result in
 * a better user experience. For example, instead of the message "4 Students are viewing this document",
 * you might display "John, Kate and 2 others are viewing this document".
 * The offset attribute allows you to offset a number by any desired value.
 * Let's take a look at an example:
 *
 * <pre>
 * <ng-pluralize count="StudentCount" offset=2
 *               when="{'0': 'Nobody is viewing.',
 *                      '1': '{{Student1}} is viewing.',
 *                      '2': '{{Student1}} and {{Student2}} are viewing.',
 *                      'one': '{{Student1}}, {{Student2}} and one other Student are viewing.',
 *                      'other': '{{Student1}}, {{Student2}} and {} other Students are viewing.'}">
 * </ng-pluralize>
 * </pre>
 *
 * Notice that we are still using two plural categories(one, other), but we added
 * three explicit number rules 0, 1 and 2.
 * When one Student, perhaps John, views the document, "John is viewing" will be shown.
 * When three Students view the document, no explicit number rule is found, so
 * an offset of 2 is taken off 3, and Angular uses 1 to decide the plural category.
 * In this case, plural category 'one' is matched and "John, Marry and one other Student are viewing"
 * is shown.
 *
    ...
    <doc:example>
      <doc:source>
        <script>
          function Ctrl($scope) {
            $scope.Student1 = 'Igor';
            $scope.Student2 = 'Misko';
            $scope.StudentCount = 1;
          }
        </script>
        <div ng-controller="Ctrl">
          Student 1:<input type="text" ng-model="Student1" value="Igor" /><br/>
          Student 2:<input type="text" ng-model="Student2" value="Misko" /><br/>
          Number of Students:<input type="text" ng-model="StudentCount" value="1" /><br/>
      ...
      <doc:scenario>
        it('should show correct pluralized string', function() {
          expect(element('.doc-example-live ng-pluralize:first').text()).
                                             toBe('1 Student is viewing.');
          expect(element('.doc-example-live ng-pluralize:last').text()).
                                                toBe('Igor is viewing.');

          using('.doc-example-live').input('StudentCount').enter('0');
          expect(element('.doc-example-live ng-pluralize:first').text()).
                                               toBe('Nobody is viewing.');
          expect(element('.doc-example-live ng-pluralize:last').text()).
                                              toBe('Nobody is viewing.');

          using('.doc-example-live').input('StudentCount').enter('2');
          expect(element('.doc-example-live ng-pluralize:first').text()).
                                            toBe('2 Students are viewing.');
          expect(element('.doc-example-live ng-pluralize:last').text()).
                              toBe('Igor and Misko are viewing.');

          using('.doc-example-live').input('StudentCount').enter('3');
          expect(element('.doc-example-live ng-pluralize:first').text()).
                                            toBe('3 Students are viewing.');
          expect(element('.doc-example-live ng-pluralize:last').text()).
                              toBe('Igor, Misko and one other Student are viewing.');

          using('.doc-example-live').input('StudentCount').enter('4');
          expect(element('.doc-example-live ng-pluralize:first').text()).
                                            toBe('4 Students are viewing.');
          expect(element('.doc-example-live ng-pluralize:last').text()).
                              toBe('Igor, Misko and 2 other Students are viewing.');
        });
        ...
        it('should show data-binded names', function() {
          ...
          using('.doc-example-live').input('Student1').enter('Di');
          using('.doc-example-live').input('Student2').enter('Vojta');
          expect(element('.doc-example-live ng-pluralize:last').text()).
              toBe('Di, Vojta and 2 other Students are viewing.');
        });
      </doc:scenario>
    </doc:example>
 */
 ...
 * then uses `ngRepeat` to display every Student:
    <doc:example>
      <doc:source>
